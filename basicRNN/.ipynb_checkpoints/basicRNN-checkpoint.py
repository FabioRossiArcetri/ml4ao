...
class MLPModel(nn.Sequential):
    def __init__(self, features_size, hidden_size, num_classes, num_layers):
        ...
        self.add_module("dense0", nn.Linear(self.features_size, self.hidden_size))
        for i in range(self.num_layers-1):
            self.add_module("dense"+str(i+1), nn.Linear(self.hidden_size, self.hidden_size))
#            self.add_module("act"+str(i+1), nn.LeakyReLU(0.1))
#            self.add_module("act"+str(i+1), nn.SiLU())
            self.add_module("act"+str(i+1), nn.ReLU())
#            self.add_module("dp"+str(i+1), nn.Dropout(0.01))
        self.add_module("denseO", nn.Linear(self.hidden_size, self.hidden_size))
#        self.add_module("dpO", nn.Dropout(0.01))
        self.add_module("output", nn.Linear(self.hidden_size, self.num_classes))
    
class SimpleGRU(nn.Module):
    def __init__(self, features_size, hidden_size, num_classes, num_layers, sequence_length):
        super(SimpleGRU, self).__init__()
        self.hidden_size  = hidden_size
        self.num_layers = num_layers
        
        self.gru = nn.GRU(features_size, hidden_size, num_layers, batch_first=True, dropout=0.01)
...
class TimeSeriesDataLoader:
    ...
    def __init__(self, model_type, data_path, labels_path, timesteps, features_size, timeseries, output_indices, test_split=0.2, batch_size=32):
    ...
    def _load_data(self):
        ...
        dataOrig = data[:, 20:, :self.features_size]
#        idx = torch.randperm(dataOrig.shape[0])
#        dataOrig = dataOrig[idx].view(dataOrig.size()).clone().detach()
        ...
        self.labels = labels[:,self.output_indices]
...
class trainingService(object):
    ...
    def loadData(self):
        ...
        self.data_loader = TimeSeriesDataLoader(self.model_type,
                                           ...
                                           self.time_series,
                                           self.output_indices,
                                           test_split=self.splitFactor, batch_size=self.batch_size)
    ...
    def setupModel(self):
        ...
        elif self.model_type=='SGRU':
            self.model = SimpleGRU(self.features_size, self.hidden_size, self.output_size, self.hidden_layers, self.time_points).to(device)
        elif self.model_type=='RNN':
            self.model = RNNModel(self.features_size, self.hidden_size, self.output_size, self.hidden_layers).to(device)
        elif self.model_type=='LSTM':
            self.model = MultivariateRNN(self.features_size, self.hidden_size, self.output_size, self.hidden_layers).to(device)
